...
export default function ForecastCard({ weather, index, setWeatherIndex, delay, className }) {
  return (
    <motion.button initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0, transition: { delay: delay } }} exit={{ opacity: 0, y: 20 }} transition={{ ease: 'easeInOut' }} className={`px-5 py-4 bg-zinc-700 border-2 rounded-xl text-xl text-white font-bold ${className}`} onClick={() => setWeatherIndex(index)}>
      <div className='grid grid-rows-2 items-stretch'>
        <div className='flex'>
          ...
          <div className='font-medium font-mono text-right ml-auto'>
            <p className='text-white text-2xl'>{DateTime.fromSeconds(weather.dt).setLocale('ru').toFormat('cccc')}</p>
